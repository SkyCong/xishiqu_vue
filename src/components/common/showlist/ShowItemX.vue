  ...
  <div class="node-list" ref="tab"> 
    ...
    <template v-if="pinyinName!='film'">
      <div class="wrapper" :style="'width: '+1.12*(mData.length-1)+'rem;'">
        <router-link :to="'/activity/'+item.pinyinName"  class="node node--activity vertical" v-for="(item , n) in mData.slice(1)" :key="n.actCode" tag="div">
    ...
    <template v-else>
        <div class="wrapper" :style="'width: '+1.12*mData.length+'rem;'">
          <!-- <router-link> -->
            <router-link :to="'/filmdis/'+film.filmId" class="node node--film vertical" v-for="(film , t) in mData" :key="t.filmId" tag="div">
